...
int solveILP (Game* game, int opCode, int x, int y){
    GRBenv   *env   = NULL;
    GRBmodel *model = NULL;
    int transCounter=0,indCounter=0,i,j,k;
    Point block, cell,p;
    int N=(*game).m*(*game).n;
    int varNum = (*game).cellsToFill;
    int       error = 0;
    LinkedList** rowsCon = (LinkedList**)malloc(N* sizeof(LinkedList*));
    LinkedList** colsCon = (LinkedList**)malloc(N* sizeof(LinkedList*));
    LinkedList** blocksCon = (LinkedList**)malloc(N* sizeof(LinkedList*));
    double*    sol = (double*)malloc(varNum*sizeof(double)); /*calculated values for variables*/
    int*       ind = (int*)malloc(varNum*sizeof(int)); /*variable array*/
    double*    val = (double*)malloc(varNum*sizeof(double)); /*coefficients of constraint*/
    double*    obj = (double*)malloc(varNum*sizeof(double)); /*coefficients of target function*/
    char*      vtype = (char*)malloc(varNum*sizeof(char)); /*type of variable*/
    ...
    Truple*  transArray = (Truple*)malloc(varNum*sizeof(Truple));
            ...
            if((*game).board.board[block.y][block.x].block[cell.y][cell.x].val==0){
                for (k=0;k<N;k++){
                    if (isValidValue(j,i,k,game)==1){
                        ...
                        p = {transCounter,transCounter,0,k};
                        insert(rowsCon[k],p)
                        rowsCon[i]
                ...
                if(indCounter>0) {
                    /* add constraint to model - note size indCounter + operator GRB_LESS_EQUAL */
                    /* -- equation value (5.0) + unique constraint name */
                    error = GRBaddconstr(model, indCounter, ind, val, GRB_LESS_EQUAL, 5, "c0");
                    if (error) {
